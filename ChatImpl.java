...
public  class ChatImpl implements ChatService {
    ...
    private void fillHistory(){
      ...
      try {
            ...
            if(line!=null) historyArray.add(line);
            while (line != null) {
	...
	public void join(InfoClient client)  throws RemoteException, Exception{
        ...
        if(isUserJoined(client)) throw new Exception("user already exists"); //user already exists
    ...
    public void leave(InfoClient client) throws RemoteException, Exception{
        ...
        if(!isUserJoined(client)) throw new Exception("This user doesn't exist"); //user doesn't exist
    ...
    public void send(String message, InfoClient sender) throws RemoteException, Exception{
        if(!isUserJoined(sender)) throw new Exception("You are not in the chatgroup");
    ...
    public ArrayList<String> getHistory(InfoClient client) throws RemoteException, Exception{
        if(!isUserJoined(client)) throw new Exception("error: you're not in the group"); 
        return historyArray;
    }
